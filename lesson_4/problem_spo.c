...
void stack_push(struct stacknode** stack, struct tree_t* node)
{
...
struct tree_t* stack_pop(struct stacknode** top)
{
...
void print_pre(struct tree_t *top)
{
  ...
  stack_push(&s, top);
	while(s != NULL)
  { 
    // pop item, print it, push its right child, push its left child
    struct tree_t* t = stack_pop(&s);
    if(t != NULL)
      printf("%d ", t->data);
    if(t->right != NULL)
      stack_push(&s, t->right);
    if(t->left != NULL)
      stack_push(&s, t->left);
  }
}
