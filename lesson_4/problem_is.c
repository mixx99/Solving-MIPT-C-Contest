...
int check_is_bst_helper(struct tree_t *top, int min, int max) {
  if (top == NULL)
    return 1;
  ...
  return check_is_bst_helper(top->left, min, top->data) &&
         check_is_bst_helper(top->right, top->data, max);
}

int check_is_bst(struct tree_t *top) {
  return check_is_bst_helper(top, INT_MIN, INT_MAX);
}
